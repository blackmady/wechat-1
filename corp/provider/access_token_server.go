...
type AccessTokenServer interface {
	// 从中控服务器获取被缓存的 provider_access_token.
	Token() (string, error)

	// 请求中控服务器到微信服务器刷新 provider_access_token.
	//
	//  高并发场景下某个时间点可能有很多请求(比如缓存的 provider_access_token 刚好过期时), 但是我们
	//  不期望也没有必要让这些请求都去微信服务器获取 provider_access_token(有可能导致api超过调用限制),
	//  实际上这些请求只需要一个新的 provider_access_token 即可, 所以建议 AccessTokenServer 从微信服务器
	//  获取一次 provider_access_token 之后的至多5秒内(收敛时间, 视情况而定, 理论上至多5个http或tcp周期)
...
func NewDefaultAccessTokenServer(corpid, providerSecret string, clt *http.Client) (srv *DefaultAccessTokenServer) {
	if clt == nil {
...
// 从微信服务器获取 provider_access_token.
...
func (srv *DefaultAccessTokenServer) getToken() (token accessTokenInfo, cached bool, err error) {
	...
	// 在收敛周期内直接返回最近一次获取的 provider_access_token, 这里的收敛时间设定为4秒.
	...
	// 由于网络的延时, provider_access_token 过期时间留了一个缓冲区
	switch {
